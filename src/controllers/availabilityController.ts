...
export class AvailabilityController {
   ...
   */
  static async getEmployeeStatuses(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  static async updateEmployeeStatus(req: Request, res: Response): Promise<void> {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        res.status(400).json({
   ...
   */
  static async getDepartmentUtilization(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;

      // Verify department exists
      const deptCheck = await this.pool.query(
   ...
   */
  private static mapAvailabilityRow(row: any): EmployeeAvailability {
    return {
      id: row.id,
