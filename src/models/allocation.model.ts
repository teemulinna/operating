    ...
    try {
      ...
      if (overlaps.length > 0) {
        throw new DatabaseError(
          `Allocation conflicts detected with ${overlaps.length} existing allocation(s). Use force parameter to override.`
        );
      }
      ...
      if (error.code === '23514') { // Check constraint violation
        throw new DatabaseError('Invalid allocation data - check dates and hours');
      }
      throw error;
    }
  }
