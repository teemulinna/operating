...
export class PipelineManagementService {
  ...
  async getPipelineAnalytics(filters: Partial<PipelineFilters>): Promise<PipelineAnalytics & { averageProbability: number; projectsByStage: Record<string, number>; winRate: number; averageCycleTime: number; topClients: Array<{ name: string; value: number; count: number }> }> {
    ...
    // Calculate real average cycle time from completed projects
  ...
  private async calculateAverageCycleTime(): Promise<number> {
    try {
      const query = `
        SELECT
      ...
      // If no data available, calculate from all projects with dates
      if (!avgCycleTime || completedCount === 0) {
        ...
        return parseFloat(fallbackResult.rows[0].estimated_avg_duration) || 45; // Default 45 days
      }
