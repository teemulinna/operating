...
import { 
  SkillRequirement, 
  CreateSkillRequirementInput, 
  UpdateSkillRequirementInput,
  SkillRequirementFilters,
  ProficiencyLevel,
  RequirementPriority,
  Skill,
  DatabaseError
...
export class SkillRequirementModel {
  ...
  static async create(input: CreateSkillRequirementInput): Promise<SkillRequirement> {
    try {
      const query = `
      ...
      if (error.code === '23505') { // Unique constraint violation
        throw new DatabaseError('Skill requirement already exists for this project and skill');
      }
      if (error.code === '23503') { // Foreign key constraint violation
        throw new DatabaseError('Invalid project ID or skill ID');
      }
      throw error;
    }
  }

  static async findById(id: string): Promise<SkillRequirement | null> {
    const query = `
  ...
  static async findByIdWithSkill(id: string): Promise<SkillRequirement & { skill: Skill } | null> {
    const query = `
      SELECT 
        sr.*,
        json_build_object(
  ...
  static async findByProject(projectId: string): Promise<Array<SkillRequirement & { skill: Skill }>> {
    const query = `
      SELECT 
        sr.*,
        json_build_object(
  ...
  static async findBySkill(skillId: string): Promise<Array<SkillRequirement & { project: { id: string; name: string; status: string; priority: string } }>> {
    const query = `
      SELECT 
        sr.*,
        json_build_object(
  ...
  static async findAll(filters: SkillRequirementFilters = {}): Promise<SkillRequirement[]> {
  ...
  static async update(id: string, updates: UpdateSkillRequirementInput): Promise<SkillRequirement> {
  ...
  static async delete(id: string): Promise<SkillRequirement> {
    const query = `
  ...
  static async getSkillGapAnalysis(projectId: string): Promise<Array<{
    requirement: SkillRequirement & { skill: Skill };
    ...
    return result.rows.map(row => ({
      requirement: {
        ...
        minimumProficiency: row.minimum_proficiency as ProficiencyLevel,
  ...
  static async updateFulfillmentStatus(projectId: string): Promise<void> {
    const query = `
      UPDATE skill_requirements 
      SET fulfilled = (
  ...
  static async getSkillDemandAnalysis(): Promise<Array<{
    skill: Skill;
    totalDemand: number;
    ...
    return result.rows.map(row => ({
      ...
      shortageRisk: row.shortage_risk as 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'
    }));
  }

  private static mapRow(row: any): SkillRequirement {
    return {
      id: row.id,
      projectId: row.project_id,
      skillId: row.skill_id,
