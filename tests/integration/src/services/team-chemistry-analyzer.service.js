...
exports.TeamChemistryAnalyzerService = void 0;
const database_service_1 = require("../database/database.service");
const database_factory_1 = require("../database/database-factory");
const api_error_1 = require("../utils/api-error");
class TeamChemistryAnalyzerService {
    constructor(db) {
     ...
     */
    async analyzeTeamChemistry(teamComposition) {
        try {
            const analysis = {
            ...
            throw new api_error_1.ApiError(500, 'Failed to analyze team chemistry');
        }
    }
    /**
     * Analyze skill complementarity within the team
     */
    async analyzeSkillComplementarity(team) {
        const allSkills = new Map(); // skillId -> Set of employeeIds
        const employeeSkills = new Map(); // employeeId -> Set of skillIds
        ...
        if (team.roleRequirements) {
            for (const role of team.roleRequirements) {
                for (const requiredSkill of role.skillRequirements) {
                    const hasSkill = team.members.some(member => member.skills.some(skill => skill.skillName.toLowerCase() === requiredSkill.toLowerCase()));
                    if (!hasSkill) {
     ...
     */
    async analyzeExperienceDiversity(team) {
        const seniorityBalance = { junior: 0, mid: 0, senior: 0, lead: 0 };
        const domainExpertise = new Map();
     ...
     */
    async analyzeCollaborationHistory(team) {
     ...
     */
    async analyzeCommunicationDynamics(team) {
        const workingStyles = [];
     ...
     */
    async identifyRiskFactors(team) {
        const riskFactors = [];
        // Check for skill gaps
        if (team.roleRequirements) {
            for (const role of team.roleRequirements) {
                const missingSkills = role.skillRequirements.filter(requiredSkill => !team.members.some(member => member.skills.some(skill => skill.skillName.toLowerCase() === requiredSkill.toLowerCase() &&
     ...
     */
    async predictTeamPerformance(team) {
        ...
        const velocityScore = Math.min(100, (avgSeniority * 20) +
        ...
        const avgSkillLevel = team.members.reduce((sum, m) => sum + (m.skills.reduce((skillSum, s) => skillSum + s.proficiencyLevel, 0) / Math.max(1, m.skills.length)), 0) / teamSize;
        ...
        const stabilityScore = Math.min(100, (teamSize >= 3 && teamSize <= 7 ? 30 : 15) +
     ...
     */
    async generateOptimizationSuggestions(analysis, team) {
        const suggestions = [];
    ...
    calculateProjectSuccessScore(avgHours, duration) {
    ...
    calculateStyleBalance(workingStyles) {
    ...
    calculateOverallScore(analysis) {
        ...
        const avgPerformance = (analysis.predictedPerformance.velocityScore +
            analysis.predictedPerformance.qualityScore +
            analysis.predictedPerformance.innovationScore +
            analysis.predictedPerformance.stabilityScore) / 4;
        return Math.round(analysis.skillComplementarity.score * weights.skillComplementarity +
    ...
    async identifyPotentialConflicts(team) {
...
exports.TeamChemistryAnalyzerService = TeamChemistryAnalyzerService;
