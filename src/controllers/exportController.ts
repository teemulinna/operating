...
export class ExportController {
  ...
  static async exportEmployeesExcel(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Add headers
      const headers = fields.map((field: any) => fieldMapping[field] || field);
      ...
      worksheet.columns.forEach(column => {
        if (column.header) {
          const maxLength = Math.max(
            ...
            ...column.values?.map(v => (v ? v.toString().length : 0)) || [0]
          );
   ...
   */
  private static async syncWithJira(_syncType: string, data: any): Promise<any> {
   ...
   */
  private static async syncWithAsana(_syncType: string, data: any): Promise<any> {
   ...
   */
  private static async syncWithTrello(_syncType: string, data: any): Promise<any> {
