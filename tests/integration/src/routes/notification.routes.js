...
const slackService = new slack_service_1.SlackService();
const pushService = new push_service_1.PushService();
...
router.post('/batch/mark-read', async (req, res) => {
    try {
        ...
        const promises = notificationIds.map(id => notificationController.notificationService.markAsRead(id, userId));
...
router.get('/analytics/delivery', async (req, res) => {
    try {
      ...
      WHERE n.recipient_id = $1 
        AND ndl.attempted_at >= NOW() - INTERVAL '${parseInt(days)} days'
