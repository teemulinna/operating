...
class ReportingService {
    ...
    `;
        ...
        const { AnalyticsService } = await Promise.resolve().then(() => require('./analytics.service'));
        ...
        const recommendations = this.generateUtilizationRecommendations(employeeResult.rows, departmentData.data);
        const data = {
            summary: {
     ...
     */
    async generateBurnDownReport(projectId) {
        const startTime = Date.now();
        // Get project details
        const projectQuery = `
      SELECT p.*, 
    ...
    `;
        ...
        const currentStatus = {
            ...
            scheduleStatus: this.calculateScheduleStatus(latestSnapshot?.completion_percentage || 0, new Date(), new Date(project.start_date), new Date(project.end_date)),
            estimatedCompletionDate: this.estimateCompletionDate(latestSnapshot?.completion_percentage || 0, latestSnapshot?.burn_rate || 0, new Date(project.end_date)),
     ...
     */
    async generateComparisonReport(type, subjectIds, metrics, filters = {}) {
        const startTime = Date.now();
        let subjects = [];
        if (type === 'department') {
     ...
     */
    async exportReport(reportData, reportType, exportOptions) {
    ...
    generateUtilizationRecommendations(employeeData, departmentData) {
    ...
    calculateBudgetStatus(actualSpend, plannedBudget) {
        if (actualSpend > plannedBudget)
            return 'over-budget';
        if (actualSpend > plannedBudget * 0.8)
            return 'approaching-budget';
        return 'on-budget';
    }
    calculateScheduleStatus(completionPercentage, currentDate, startDate, endDate) {
        const totalDuration = endDate.getTime() - startDate.getTime();
        const elapsed = currentDate.getTime() - startDate.getTime();
        const expectedCompletion = (elapsed / totalDuration) * 100;
        if (completionPercentage < expectedCompletion - 5)
            return 'behind-schedule';
        if (completionPercentage > expectedCompletion + 5)
            return 'ahead-of-schedule';
        return 'on-schedule';
    }
    estimateCompletionDate(completionPercentage, burnRate, plannedEndDate) {
        if (completionPercentage >= 100 || burnRate <= 0)
    ...
    generateProjectForecasts(snapshots, project) {
    ...
    async getDepartmentMetricsForComparison(departmentIds, metrics, filters) {
        // Implementation for getting department metrics
        return [];
    }
    async getProjectMetricsForComparison(projectIds, metrics, filters) {
        // Implementation for getting project metrics
        return [];
    }
    async getEmployeeMetricsForComparison(employeeIds, metrics, filters) {
        // Implementation for getting employee metrics
        return [];
    }
    calculateComparisons(subjects, metrics) {
        // Implementation for calculating metric comparisons
        return [];
    }
    generateComparisonInsights(comparisons, type) {
        // Implementation for generating insights from comparisons
        return [];
    }
    generateComparisonRecommendations(comparisons, subjects) {
        // Implementation for generating recommendations
        return [];
    }
    async convertToCSV(reportData, reportType) {
        // Implementation for CSV conversion
        return 'csv content';
    }
    async generatePDF(reportData, reportType, options) {
