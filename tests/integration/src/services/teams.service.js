...
class TeamsService {
    ...
    async sendNotification(notification) {
        try {
            ...
            await axios_1.default.post(this.webhookUrl, teamsMessage);
    ...
    formatTeamsMessage(notification) {
        ...
        if (actions && Array.isArray(actions) && actions.length > 0) {
            const potentialActions = actions.slice(0, 4).map((action) => ({
                "@type": "OpenUri",
                name: action.label,
                targets: [
                    {
    ...
    async sendAdaptiveCard(notification) {
        try {
            if (!this.webhookUrl) {
            ...
            await axios_1.default.post(this.webhookUrl, adaptiveCard);
    ...
    formatAdaptiveCard(notification) {
        ...
        card.attachments[0].content.body.push(metadataFacts);
        // Add actions
        const actions = notification.metadata?.actions;
        if (actions && Array.isArray(actions) && actions.length > 0) {
            const cardActions = actions.slice(0, 4).map((action) => ({
                type: "Action.OpenUrl",
                title: action.label,
                url: `${process.env.APP_URL}/notifications/${notification.id}/actions/${action.action}`
            }));
            card.attachments[0].content.actions = cardActions;
        }
        else {
    ...
    async testConnection() {
        try {
            ...
            await axios_1.default.post(this.webhookUrl, testCard);
            return true;
        }
        catch (error) {
    ...
    async sendBulkNotification(notifications) {
        try {
            ...
            await axios_1.default.post(this.webhookUrl, digestCard);
