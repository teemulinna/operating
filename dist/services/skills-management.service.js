...
exports.SkillsManagementService = void 0;
...
class SkillsManagementService {
    ...
    async assignSkillToEmployee(employeeId, skillAssignment) {
    ...
    async performSkillGapAnalysis(employeeId) {
        const employeeQuery = `
    ...
    `;
        ...
        const currentSkillMap = new Map(currentSkills.rows.map(row => [row.skill_id, row.proficiency_level]));
        ...
        for (const peerSkill of peerSkills.rows) {
            ...
            if (currentLevel === 0) {
                ...
                recommendations.push({
                    ...
                    suggestedResources: await this.getSuggestedResources(peerSkill.skill_name, currentLevel < 3 ? 'intermediate' : 'advanced')
                });
            }
        }
        return {
    ...
    async getSkillAnalytics() {
       ...
       LIMIT 10`
        ...
        const [distResult, proficiencyResult, categoryResult, emergingResult] = await Promise.all(analyticsQueries.map(query => this.db.query(query)));
    ...
    calculatePriority(peerCount, level) {
    ...
    async getSuggestedResources(skillName, difficulty) {
        const resources = [
            {
                type: 'online_course',
...
exports.SkillsManagementService = SkillsManagementService;
//# sourceMappingURL=skills-management.service.js.map