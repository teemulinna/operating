...
exports.HistoricalDataAggregator = void 0;
/**
...
class HistoricalDataAggregator {
     ...
     */
    async aggregateResourceUtilization(config) {
     ...
     */
    async aggregateProjectPerformance(config) {
     ...
     */
    async getResourceHistory(employeeId, timeRange) {
     ...
     */
    async detectPatterns(data, patternTypes = ['seasonal', 'trend']) {
        const patterns = [];
     ...
     */
    async aggregateSkillDemand(skills, timeRange) {
        const skillDemand = {};
      ...
      `;
            ...
            skillDemand[skill] = rawData.rows.map((row) => ({
     ...
     */
    async calculateCapacityTrends(timeRange) {
    ...
    `;
        ...
        const availableCapacity = totalCapacity.map((capacity, index) => ({
            timestamp: capacity.timestamp,
    ...
    async generateTrainingDataset(config) {
        ...
        const featureData = await Promise.all(featureQueries.map(query => this.dbService.query(query.sql, query.params)));
    ...
    processProjectData(rawData, config) {
        return rawData.map(row => ({
            timestamp: new Date(row.period),
            value: parseFloat(row.value),
            metadata: {
    ...
    async getResourceUtilizationHistory(employeeId, timeRange) {
        const query = `
      SELECT 
    ...
    async getResourceProjectHistory(employeeId, timeRange) {
        const query = `
      SELECT 
        p.id as project_id,
        aa.start_date,
    ...
    async getResourcePerformanceHistory(employeeId, timeRange) {
    ...
    async getResourceSkills(employeeId) {
        ...
        if (results.rows.length > 0 && results.rows[0].skills) {
            return results.rows[0].skills.split(',').map((skill) => skill.trim());
        }
        return [];
    }
    async detectSeasonalPattern(data) {
        if (data.length < 14)
    ...
    async detectCyclicalPattern(data) {
    ...
    calculatePhase(values, period) {
        ...
        const maxPhase = seasonalComponents.reduce((max, current) => current.value > max.value ? current : max, seasonalComponents[0]);
    ...
    calculateCyclicalStrength(values, cycleLength) {
    ...
    getAggregationFunction(method, column) {
        switch (method) {
            case 'sum':
                return `SUM(${column})`;
            case 'average':
    ...
    async buildFeatureQueries(features, timeRange, frequency) {
        const queries = [];
    ...
    async buildTargetQuery(target, timeRange, frequency) {
        let sql = '';
        const params = [timeRange.start.toISOString(), timeRange.end.toISOString()];
        switch (target) {
    ...
    createTimeIndex(timeRange, frequency) {
        const dates = [];
    ...
    findValueForTimestamp(data, timestampStr) {
...
exports.HistoricalDataAggregator = HistoricalDataAggregator;
