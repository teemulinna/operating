...
export class DepartmentModel {
  ...
  static async create(input: CreateDepartmentInput): Promise<Department> {
    try {
      ...
      if (error.code === '23505') { // Unique constraint violation
        throw new DatabaseError(`Department with name '${input.name}' already exists`);
      }
      throw error;
    }
  }
  ...
  static async findByIdWithEmployees(id: string): Promise<DepartmentWithEmployees | null> {
    const query = `
      SELECT 
        d.*,
        json_agg(
  ...
  static async findAll(filters: { isActive?: boolean } = {}): Promise<Department[]> {
  ...
  static async update(id: string, updates: UpdateDepartmentInput): Promise<Department> {
    ...
    try {
      ...
      if (error.code === '23505') { // Unique constraint violation
        throw new DatabaseError(`Department with name '${updates.name}' already exists`);
      }
      throw error;
    }
  }
  ...
  static async delete(id: string): Promise<Department> {
    ...
    if (parseInt(employeeCheck.rows[0].count) > 0) {
      throw new DatabaseError('Cannot delete department with active employees');
    }
  ...
  private static mapRow(row: any): Department {
    return {
      id: row.id,
      name: row.name,
