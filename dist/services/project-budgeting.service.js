...
exports.ProjectBudgetingService = void 0;
const database_service_1 = require("../database/database.service");
class ProjectBudgetingService {
    constructor() {
        this.db = database_service_1.DatabaseService.getInstance();
    }
    async createProjectBudget(budgetData) {
        const calculatedTotal = budgetData.laborBudget + budgetData.nonLaborBudget;
        const tolerance = budgetData.totalBudget * 0.01;
        ...
        RETURNING *
      `;
            const versionResult = await client.query('SELECT COALESCE(MAX(version), 0) + 1 as next_version FROM project_budgets WHERE project_id = $1', [budgetData.projectId]);
    ...
    async generateBudgetForecast(projectId, forecastRequest) {
    ...
    `;
        ...
        const scenarios = {
            optimistic: {
                totalCost: forecastedTotalCost * 0.85,
                probability: 20
            },
            realistic: {
            ...
            pessimistic: {
                totalCost: forecastedTotalCost * 1.25,
                probability: 20
            }
        };
        const riskFactors = [
            {
                ...
                impact: burnRate > budget.totalBudget / 100 ? 'high' : 'medium',
                likelihood: 0.3,
    ...
    async getCostTracking(projectId, period) {
        const budgetInfo = await this.getProjectBudget(projectId);
        if (!budgetInfo) {
            throw new Error('No approved budget found for project');
        }
    ...
    `;
        ...
        const earnedValue = currentSpending;
        ...
        return {
            ...
            trends: {
                ...
                projectedCompletion: currentSpending + (budgetInfo.burnRate * 30),
                riskLevel: costVariancePercentage > 10 ? 'high' : costVariancePercentage > 5 ? 'medium' : 'low'
            }
        };
    }
    async getMarginAnalysis(projectId) {
        const budgetInfo = await this.getProjectBudget(projectId);
        if (!budgetInfo) {
            throw new Error('No approved budget found for project');
        }
    ...
    async updateRateCard(rateCardId, updates) {
        const query = `
      UPDATE rate_cards 
      SET 
...
exports.ProjectBudgetingService = ProjectBudgetingService;
//# sourceMappingURL=project-budgeting.service.js.map