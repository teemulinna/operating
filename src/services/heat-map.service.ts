...
export interface HeatMapData {
  ...
  utilizationCategory: 'critical' | 'over' | 'high' | 'optimal' | 'moderate' | 'low' | 'available';
  heatColor: string;
  lastUpdated: Date;
}
...
export class HeatMapService {
  ...
  async getHeatMapData(filters: HeatMapFilters = {}): Promise<HeatMapData[]> {
    ...
    try {
      ...
      throw new ApiError(500, 'Failed to fetch heat map data');
    }
  }
  ...
  async getHeatMapSummary(filters: HeatMapFilters = {}): Promise<HeatMapSummary> {
    ...
    const cached = await this.cache.get<HeatMapSummary>(cacheKey);
    if (cached) return cached;
    ...
    try {
      ...
      throw new ApiError(500, 'Failed to fetch heat map summary');
    }
  }
   ...
   */
  async getEmployeeTimeline(employeeId: string, startDate: Date, endDate: Date): Promise<any[]> {
    ...
    try {
      ...
      throw new ApiError(500, 'Failed to fetch employee timeline');
    }
  }
  ...
  async refreshHeatMap(): Promise<void> {
    try {
      ...
      throw new ApiError(500, 'Failed to refresh heat map data');
    }
  }
   ...
   */
  async exportToCSV(filters: HeatMapFilters = {}): Promise<string> {
   ...
   */
  private mapRowToHeatMapData(row: any): HeatMapData {
    return {
   ...
   */
  private generateCacheKey(type: string, filters: HeatMapFilters): string {
