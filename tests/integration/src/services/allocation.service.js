...
class AllocationService {
    ...
    static async checkOverAllocationWarnings(employeeId, startDate, endDate, allocatedHours) {
        ...
        for (const week of weeks) {
            const warning = await this.overAllocationService.checkWeeklyOverAllocation(employeeId, week.weekStartDate, week.weekEndDate);
            if (warning) {
    ...
    static async getOverAllocationSummary(startDate, endDate) {
        return this.overAllocationService.getScheduleViewWarnings(startDate, endDate);
    }
    /**
     * Helper method to get weeks between two dates
     */
    ...
    static async exportAllocationsToCSV(options = {}) {
        const { AllocationCSVExportService } = await Promise.resolve().then(() => require('./allocation-csv-export.service'));
...
AllocationService.overAllocationService = new over_allocation_warning_service_1.OverAllocationWarningService();
