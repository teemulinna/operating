...
export class ProjectTemplateModel {
   ...
   */
  static async create(templateData: Omit<ProjectTemplate, 'templateId' | 'createdAt' | 'updatedAt'>): Promise<ProjectTemplate> {
   ...
   * Get template by ID
   */
  static async findById(templateId: string): Promise<ProjectTemplate | null> {
    const client = await this.pool.connect();
   ...
   * Update template
   */
  static async update(templateId: string, updates: Partial<ProjectTemplate>): Promise<ProjectTemplate | null> {
    const client = await this.pool.connect();
   ...
   * Update rating
   */
  static async updateRating(templateId: string, newRating: number): Promise<ProjectTemplate | null> {
    const client = await this.pool.connect();
   ...
   */
  private static mapRowToTemplate(row: any): ProjectTemplate {
    return {
   ...
   */
  static getEstimatedProjectDuration(template: ProjectTemplate): number {
    if (template.defaultDuration) return template.defaultDuration;
    ...
    const maxTaskEnd = template.defaultTasks.reduce((max, task) => {
      const taskEnd = task.duration + Math.max(...task.dependencies.map(dep => 
        template.defaultTasks.find(t => t.id === dep)?.duration || 0
      ), 0);
  ...
  static getEstimatedBudget(template: ProjectTemplate, hourlyRate = 100): number {
    if (template.defaultBudget) return template.defaultBudget;
  ...
  static validateTemplate(template: Partial<ProjectTemplate>): string[] {
    const errors: string[] = [];
