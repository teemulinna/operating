...
export class ResourceAllocationModel {
  ...
  static async create(input: CreateResourceAllocationInput): Promise<ResourceAllocation> {
    try {
      ...
      if (error.code === '23505') { // Unique constraint violation
        throw new DatabaseError('Employee is already allocated to this project for overlapping dates');
      }
      if (error.code === '23503') { // Foreign key constraint violation
        throw new DatabaseError('Invalid project ID or employee ID');
      }
      if (error.code === '23P01') { // Serialization failure (custom check for overlapping)
        throw new DatabaseError('Employee has overlapping resource allocation for the specified period');
      }
      throw error;
    }
  }
  ...
  static async findByProject(projectId: string): Promise<Array<ResourceAllocation & { employee: Employee }>> {
    const query = `
      SELECT 
        ra.*,
        json_build_object(
  ...
  static async findByEmployee(employeeId: string): Promise<Array<ResourceAllocation & { project: Project }>> {
    const query = `
      SELECT 
        ra.*,
        json_build_object(
  ...
  static async findOverlapping(
    ...
    const values: any[] = [employeeId, startDate, endDate];
  ...
  static async getUtilizationByEmployee(
    dateFrom: Date,
    dateTo: Date,
    departmentId?: string
  ): Promise<Array<{
  ...
  static async getCapacityConflicts(
    dateFrom: Date,
    dateTo: Date
  ): Promise<Array<{
    employeeId: string;
  ...
  static async getBillableHoursSummary(
    dateFrom: Date,
    dateTo: Date
  ): Promise<Array<{
    projectId: string;
