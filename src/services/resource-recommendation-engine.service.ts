...
import { SkillMatcherService, SkillMatchCriteria, EmployeeSkillMatch } from './skill-matcher.service';
import { TeamChemistryAnalyzerService, TeamComposition, TeamChemistryAnalysis } from './team-chemistry-analyzer.service';
...
export interface RecommendationRequest {
  ...
  preferences?: {
    ...
    prioritizeTeamChemistry?: boolean;
...
export class ResourceRecommendationEngine {
  ...
  private teamAnalyzer: TeamChemistryAnalyzerService;

  constructor(db?: DatabaseService) {
    this.db = db || DatabaseService.getInstance();
    this.skillMatcher = new SkillMatcherService(this.db);
    this.teamAnalyzer = new TeamChemistryAnalyzerService(this.db);
  }

  static async create(): Promise<ResourceRecommendationEngine> {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(teamOptions.length, maxRecommendations); i++) {
        ...
        const recommendation = await this.buildRecommendation(
          ...
          request,
          filters,
          includeAlternatives ? teamOptions.slice(i + 1, i + 3) : []
        );
   ...
   * Generate multiple team composition options
   */
  private async generateTeamOptions(
    request: RecommendationRequest,
    maxOptions: number
  ): Promise<Array<{
    composition: TeamComposition;
    totalScore: number;
    members: EmployeeSkillMatch[];
  }>> {
    ...
    const roleCandidates = new Map<string, EmployeeSkillMatch[]>();
    ...
    for (const role of request.roleRequirements) {
      const criteria: SkillMatchCriteria = {
        requiredSkills: role.skillRequirements.map(sr => ({
          ...
          minimumProficiency: sr.minimumProficiency as any,
          weight: sr.weight,
    ...
    for (const strategy of strategies.slice(0, maxOptions)) {
      const teamOption = await this.generateTeamByStrategy(
        request,
        roleCandidates,
        strategy
      );
   ...
   */
  private async generateTeamByStrategy(
    request: RecommendationRequest,
    roleCandidates: Map<string, EmployeeSkillMatch[]>,
    strategy: string
  ): Promise<{
    composition: TeamComposition;
    totalScore: number;
    ...
    const selectedMembers: EmployeeSkillMatch[] = [];
    ...
    for (const role of request.roleRequirements) {
      ...
      let selectedCandidates: EmployeeSkillMatch[] = [];
  ...
  // Helper methods
  private getSeniorityScore(position: string): number {
  ...
  private selectDiverseTeam(candidates: EmployeeSkillMatch[], count: number): EmployeeSkillMatch[] {
    const selected: EmployeeSkillMatch[] = [];
  ...
  private removeDuplicateTeams(teams: Array<{
    composition: TeamComposition;
    totalScore: number;
    members: EmployeeSkillMatch[];
  }>): Array<{
  ...
  private buildReasoning(
    ...
    teamChemistry: TeamChemistryAnalysis,
    request: RecommendationRequest
  ): ResourceRecommendation['reasoning'] {
    const keyStrengths: string[] = [];
    const potentialConcerns: string[] = [];
    const tradeoffs: string[] = [];

    // Analyze strengths
    const highPerformers = teamOption.members.filter((m: any) => m.overallMatchScore >= 80).length;
    if (highPerformers > 0) {
    ...
    // Analyze concerns
    const lowPerformers = teamOption.members.filter((m: any) => m.overallMatchScore < 60).length;
    if (lowPerformers > 0) {
    ...
    // Identify tradeoffs
    const avgSeniority = teamOption.members.reduce((sum: number, m: any) => 
      sum + this.getSeniorityScore(m.employee.position), 0) / teamOption.members.length;
  ...
  private async generateOptimizations(
    ...
    teamChemistry: TeamChemistryAnalysis,
    request: RecommendationRequest
  ): Promise<ResourceRecommendation['optimizations']> {
    const optimizations: ResourceRecommendation['optimizations'] = [];
    ...
    const criticalSkillGaps = skillGaps.filter(skill => 
      request.roleRequirements.some(role =>
        role.skillRequirements.some(sr => 
          sr.skillName.toLowerCase() === skill.toLowerCase() && sr.isRequired
        )
      )
    );
  ...
  private async buildManualOverrides(
    teamOption: any,
    request: RecommendationRequest
  ): Promise<ResourceRecommendation['manualOverrides']> {
    const allowedSubstitutions: ResourceRecommendation['manualOverrides']['allowedSubstitutions'] = [];
    ...
    const roleFlexibility: ResourceRecommendation['manualOverrides']['roleFlexibility'] = 
      request.roleRequirements.map(role => ({
        roleTitle: role.roleTitle,
        flexibleSkills: role.skillRequirements.filter(sr => !sr.isRequired).map(sr => sr.skillName),
        acceptableAlternatives: [] as string[] // Could be populated with similar skills
      }));
  ...
  private determineRecommendedRole(member: EmployeeSkillMatch, roleRequirements: RecommendationRequest['roleRequirements']): string {
  ...
  private findAlternativeRoles(
    member: EmployeeSkillMatch, 
    roleRequirements: RecommendationRequest['roleRequirements'],
    currentRole: string
  ): string[] {
    const alternatives: string[] = [];
    
    for (const role of roleRequirements) {
      if (role.roleTitle === currentRole) continue;
  ...
  private identifyTradeoffs(mainOption: any, alternative: any): string[] {
    const tradeoffs: string[] = [];
    
    const mainAvgScore = mainOption.members.reduce((sum: number, m: any) => sum + m.overallMatchScore, 0) / mainOption.members.length;
    const altAvgScore = alternative.members.reduce((sum: number, m: any) => sum + m.overallMatchScore, 0) / alternative.members.length;
