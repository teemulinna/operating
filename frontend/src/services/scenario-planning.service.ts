// Scenario Planning Service with What-if Modeling
import { Pool } from 'pg';
import {
  Scenario,
  ScenarioAllocation,
...
export class ScenarioPlanningService {
  ...
  async updateScenario(request: UpdateScenarioRequest): Promise<Scenario> {
  ...
  async createScenarioAllocation(request: CreateScenarioAllocationRequest): Promise<ScenarioAllocation> {
    // Validate allocation doesn't exceed 100% for the employee in the time period
    await this.validateAllocationCapacity(
      request.employeeId,
      request.startDate,
  ...
  async getScenarioAllocations(scenarioId: string, filters: {
    projectId?: string;
    employeeId?: string;
  ...
  async updateScenarioAllocation(request: UpdateScenarioAllocationRequest): Promise<ScenarioAllocation> {
    const current = await this.getScenarioAllocationById(request.id);
    if (!current) {
      throw new Error('Scenario allocation not found');
    }

    // Validate allocation capacity if percentage or dates changed
    if (request.allocationPercentage || request.startDate || request.endDate) {
      await this.validateAllocationCapacity(
        current.employeeId,
        request.startDate || current.startDate,
        request.endDate || current.endDate || current.startDate,
        request.allocationPercentage || current.allocationPercentage,
        current.scenarioId,
        request.id
      );
    }
  ...
  async deleteScenarioAllocation(id: string): Promise<boolean> {
  ...
  async compareScenarios(scenarioAId: string, scenarioBId: string): Promise<ScenarioComparison> {
    // Check if comparison already exists and is still valid
    const existingQuery = `
  ...
  async detectResourceConflicts(scenarioId: string): Promise<TimelineConflict[]> {
    const query = `
  ...
  async analyzeSkillGaps(scenarioId: string): Promise<SkillGap[]> {
    const query = `
  ...
  async duplicateScenario(scenarioId: string, newName: string): Promise<Scenario> {
  ...
  private async validateAllocationCapacity(
    employeeId: string,
    startDate: string,
    endDate: string,
    ...
    if (currentTotal + allocationPercentage > 100) {
      throw new Error(
        `Employee allocation would exceed 100% capacity. Current: ${currentTotal}%, Requested: ${allocationPercentage}%`
      );
    }
  }

  private async getScenarioAllocationById(id: string): Promise<ScenarioAllocation | null> {
  ...
  private async generateComparisonMetrics(scenarioAId: string, scenarioBId: string): Promise<any> {
    // This would include complex calculations comparing:
    // - Total costs
  ...
  private transformScenario(row: any): Scenario {
    return {
      id: row.id,
      name: row.name,
  ...
  private transformScenarioAllocation(row: any): ScenarioAllocation {
    return {
      id: row.id,
  ...
  private transformScenarioAllocationWithJoins(row: any): ScenarioAllocation {
  ...
  private transformScenarioComparison(row: any): ScenarioComparison {
    return {
      id: row.id,
