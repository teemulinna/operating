...
class JiraAdapter extends base_crm_adapter_1.BaseCRMAdapter {
    constructor(config) {
        super(config);
    ...
    async listProjects(filters) {
        try {
            ...
            return response.data.issues.map((issue) => this.mapJiraIssueToProject(issue));
        }
        catch (error) {
