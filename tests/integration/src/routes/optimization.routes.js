    ...
    `;
        ...
        for (const allocation of allocationsResult.rows) {
            ...
            if (current.totalUtilization > maxUtilization) {
                conflicts.push({
                    ...
                    affectedProjects: current.assignments.map((a) => a.projectName)
                });
            }
        }
        // Calculate average skill match per employee
        for (const [empId, data] of utilizationMap) {
            const totalSkillMatch = data.assignments.reduce((sum, a) => sum + a.skillMatch, 0);
        ...
        for (const conflict of conflicts) {
            ...
            suggestions.push({
                ...
                recommendations: lowestSkillMatches.map((a) => ({
        ...
        const totalUtilization = Array.from(utilizationMap.values())
            .reduce((sum, emp) => sum + emp.totalUtilization, 0);
        ...
        const averageSkillMatch = Array.from(utilizationMap.values())
            .reduce((sum, emp) => sum + emp.averageSkillMatch, 0) / totalEmployees || 0;
        return res.json({
            success: true,
            data: {
...
router.get('/utilization-analysis', async (req, res) => {
    try {
        await db.connect();
    ...
    `;
        ...
        const overallocated = utilizationData.filter((e) => e.status === 'overallocated').length;
        const underutilized = utilizationData.filter((e) => e.status === 'underutilized').length;
        const avgUtilization = utilizationData.reduce((sum, e) => sum + e.currentUtilization, 0) / totalEmployees || 0;
        const totalCapacity = totalEmployees * 100;
        const usedCapacity = utilizationData.reduce((sum, e) => sum + e.currentUtilization, 0);
        return res.json({
            success: true,
            data: {
                summary: {
