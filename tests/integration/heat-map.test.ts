/**
 * Integration tests for Heat Map API - Phase 1
 * Following plan.md and TDD principles
 * Testing actual production endpoints with real database
 */
...
describe('Heat Map API - Production Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      // View might already exist with proper definition, continue
    }

    // Clean up any existing cache intervals
    if ((global as any).cacheIntervals) {
      (global as any).cacheIntervals.forEach((interval: NodeJS.Timeout) => clearInterval(interval));
      (global as any).cacheIntervals = [];
    }
    ...
    try {
      ...
      await db.query(
        `INSERT INTO resource_allocations (employee_id, project_id, allocated_hours, start_date, end_date, is_active)
      ...
      await db.query(
        `INSERT INTO availability_patterns (employee_id, pattern_type, name, start_date, end_date, is_active, weekly_hours)
  ...
  describe('GET /api/capacity/heatmap', () => {
    ...
    it('should filter by employee ID', async () => {
      ...
      if (data.length > 0) {
        expect(data.every((item: any) => item.employeeId === testEmployeeId)).toBe(true);
      }
    });
    ...
    it('should filter by department ID', async () => {
      ...
      const data = response.body.data;
      if (data.length > 0) {
        expect(data.every((item: any) => item.departmentId === testDepartmentId)).toBe(true);
      }
    });
    ...
    it('should filter by utilization levels', async () => {
      ...
      if (data.length > 0) {
        expect(data.every((item: any) =>
          ['high', 'critical'].includes(item.utilizationCategory)
        )).toBe(true);
      }
    });
