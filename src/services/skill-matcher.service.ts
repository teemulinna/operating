...
import { EmployeeWithSkills, ProficiencyLevel, SkillCategory } from '../types';
...
export interface EmployeeSkillMatch {
  ...
  skillMatches: Array<{
    ...
    proficiencyGap: number; // negative = exceeds requirement
    matchScore: number; // 0-100 for this specific skill
    yearsOfExperience?: number;
    lastUsed?: Date;
  }>;
  strengthAreas: string[]; // Skills where employee exceeds requirements
  gapAreas: string[]; // Skills missing or below requirement
  availabilityScore: number; // 0-100 based on current allocation
  teamFitScore: number; // 0-100 based on past collaborations
...
export interface SkillMatchFilters {
  ...
  availabilityThreshold?: number; // minimum availability percentage
  includeBenchWarming?: boolean; // include over-qualified candidates
}
...
export class SkillMatcherService {
  ...
  static async create(): Promise<SkillMatcherService> {
  ...
  async findResourceMatches(
    ...
    try {
      ...
      const matches: EmployeeSkillMatch[] = [];
      ...
      throw new ApiError(500, 'Failed to find resource matches');
    }
  }
  ...
  async calculateSkillMatch(
    ...
    const skillMatches: EmployeeSkillMatch['skillMatches'] = [];
    ...
    for (const requiredSkill of criteria.requiredSkills) {
      ...
      const employeeProficiency = (employeeSkill as any)?.proficiencyLevel || null;
      ...
      skillMatches.push({
        ...
        proficiencyGap,
        matchScore,
        yearsOfExperience: (employeeSkill as any)?.yearsOfExperience,
        lastUsed: (employeeSkill as any)?.lastUsed
      });
    ...
    const overallMatchScore = Math.round(
      (baseScore * 0.6 + availabilityScore * 0.2 + teamFitScore * 0.2) * 
    ...
    let overallRecommendation: EmployeeSkillMatch['overallRecommendation'] = 'poor';
    if (overallMatchScore >= 85) overallRecommendation = 'excellent';
    else if (overallMatchScore >= 70) overallRecommendation = 'good';
    else if (overallMatchScore >= 50) overallRecommendation = 'fair';

    // Generate reasoning notes
    const reasoningNotes = this.generateReasoningNotes(
      skillMatches,
      availabilityScore,
      teamFitScore,
      requiredSkillsMet,
  ...
  private async calculateTeamFitScore(
    ...
    try {
      ...
      if (similarProjects >= 5) return 90;
      if (similarProjects >= 3) return 80;
      if (similarProjects >= 1) return 70;
   ...
   */
  private async getCandidatesWithSkills(filters: SkillMatchFilters): Promise<any[]> {
   ...
   */
  private generateReasoningNotes(
    skillMatches: EmployeeSkillMatch['skillMatches'],
   ...
   */
  async getMatchStatistics(criteria: SkillMatchCriteria): Promise<{
    ...
    const candidatesWithAnySkill = matches.filter(m => 
      m.skillMatches.some(sm => sm.hasSkill)
    ).length;
    
    const candidatesWithAllRequiredSkills = matches.filter(m =>
      m.skillMatches.filter(sm => sm.required).every(sm => sm.hasSkill && sm.proficiencyGap >= 0)
    ).length;
