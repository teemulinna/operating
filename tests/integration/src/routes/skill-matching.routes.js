...
const skill_matcher_service_1 = require("../services/skill-matcher.service");
const team_chemistry_analyzer_service_1 = require("../services/team-chemistry-analyzer.service");
const resource_recommendation_engine_service_1 = require("../services/resource-recommendation-engine.service");
...
const validateSkillMatchRequest = [
    ...
    (0, express_validator_1.body)('requiredSkills.*.skillId')
        .notEmpty()
        .withMessage('skillId is required for each skill'),
    (0, express_validator_1.body)('requiredSkills.*.skillName')
        .notEmpty()
        .withMessage('skillName is required for each skill'),
    (0, express_validator_1.body)('requiredSkills.*.minimumProficiency')
        .isInt({ min: 1, max: 5 })
        .withMessage('minimumProficiency must be between 1 and 5'),
    (0, express_validator_1.body)('requiredSkills.*.weight')
        .isFloat({ min: 0, max: 10 })
        .withMessage('weight must be between 0 and 10'),
    (0, express_validator_1.body)('requiredSkills.*.isRequired')
        .isBoolean()
    ...
    (0, express_validator_1.body)('roleTitle')
        .optional()
        .isString()
    ...
    (0, express_validator_1.body)('availabilityHours')
        .optional()
        .isFloat({ min: 0 })
...
const validateTeamAnalysisRequest = [
    ...
    (0, express_validator_1.body)('members.*.employeeId')
        .notEmpty()
    ...
    (0, express_validator_1.body)('members.*.position')
        .notEmpty()
        .withMessage('position is required for each member'),
    (0, express_validator_1.body)('members.*.departmentId')
        .notEmpty()
        .withMessage('departmentId is required for each member'),
    (0, express_validator_1.body)('members.*.skills')
        .isArray()
...
const validateRecommendationRequest = [
    (0, express_validator_1.body)('roleRequirements')
        .isArray({ min: 1 })
        .withMessage('roleRequirements must be a non-empty array'),
    (0, express_validator_1.body)('roleRequirements.*.roleTitle')
        .notEmpty()
        .withMessage('roleTitle is required for each role'),
    (0, express_validator_1.body)('roleRequirements.*.skillRequirements')
        .isArray({ min: 1 })
        .withMessage('skillRequirements must be a non-empty array for each role'),
    (0, express_validator_1.body)('roleRequirements.*.count')
        .isInt({ min: 1 })
        .withMessage('count must be a positive integer for each role'),
    (0, express_validator_1.body)('projectConstraints.startDate')
        .optional()
        .isISO8601()
        .toDate()
        .withMessage('startDate must be a valid ISO 8601 date'),
    (0, express_validator_1.body)('projectConstraints.endDate')
        .optional()
        .isISO8601()
        .toDate()
        .withMessage('endDate must be a valid ISO 8601 date'),
    (0, express_validator_1.body)('projectConstraints.totalBudget')
        .optional()
        .isFloat({ min: 0 })
        .withMessage('totalBudget must be a positive number'),
    (0, express_validator_1.body)('projectConstraints.maxTeamSize')
        .optional()
        .isInt({ min: 1 })
        .withMessage('maxTeamSize must be a positive integer'),
    (0, express_validator_1.body)('projectConstraints.minTeamSize')
        .optional()
        .isInt({ min: 1 })
...
router.post('/recommend', validateRecommendationRequest, (0, async_handler_1.asyncHandler)(async (req, res) => {
...
router.post('/skill-match', validateSkillMatchRequest, (0, async_handler_1.asyncHandler)(async (req, res) => {
    const validationError = checkValidationErrors(req, res);
    if (validationError)
        return validationError;
...
router.get('/score/:employeeId', (0, express_validator_1.param)('employeeId').notEmpty().withMessage('Employee ID is required'), validateSkillMatchRequest, (0, async_handler_1.asyncHandler)(async (req, res) => {
    const validationError = checkValidationErrors(req, res);
    if (validationError)
        return validationError;
    ...
    const candidates = await skillMatcher.findResourceMatches(req.body, { maxResults: 1000 });
...
router.post('/analyze-team', validateTeamAnalysisRequest, (0, async_handler_1.asyncHandler)(async (req, res) => {
    const validationError = checkValidationErrors(req, res);
    if (validationError)
        return validationError;
    const teamAnalyzer = await team_chemistry_analyzer_service_1.TeamChemistryAnalyzerService.create();
...
router.post('/optimize-team', validateTeamAnalysisRequest, (0, async_handler_1.asyncHandler)(async (req, res) => {
    const validationError = checkValidationErrors(req, res);
    if (validationError)
        return validationError;
    const teamAnalyzer = await team_chemistry_analyzer_service_1.TeamChemistryAnalyzerService.create();
...
router.get('/statistics', (0, express_validator_1.query)('projectId').optional().isString(), (0, async_handler_1.asyncHandler)(async (req, res) => {
    const validationError = checkValidationErrors(req, res);
    if (validationError)
        return validationError;
...
router.get('/skills/gaps', (0, express_validator_1.query)('departmentId').optional().isString(), (0, express_validator_1.query)('projectType').optional().isString(), (0, async_handler_1.asyncHandler)(async (req, res) => {
    const skillMatcher = await skill_matcher_service_1.SkillMatcherService.create();
...
router.post('/bulk-score', validateSkillMatchRequest, (0, express_validator_1.body)('employeeIds').isArray({ min: 1 }).withMessage('employeeIds must be a non-empty array'), (0, async_handler_1.asyncHandler)(async (req, res) => {
    const validationError = checkValidationErrors(req, res);
    if (validationError)
        return validationError;
    const skillMatcher = await skill_matcher_service_1.SkillMatcherService.create();
    ...
    return res.json({
        ...
        data: {
            ...
            notFound: employeeIds.filter((id) => !requestedMatches.some(m => m.employeeId === id))
        }
    });
}));
// GET /api/matching/recommendations/:recommendationId - Get specific recommendation details
router.get('/recommendations/:recommendationId', (0, express_validator_1.param)('recommendationId').notEmpty().withMessage('Recommendation ID is required'), (0, async_handler_1.asyncHandler)(async (req, res) => {
