...
router.post('/direct',
  ...
  [
    ...
    body('allocatedHours').isFloat({ min: 0.1, max: 168 }).withMessage('Allocated hours must be between 0.1 and 168'),
    body('roleOnProject').optional().isString().withMessage('Role must be a string'),
    body('startDate').isISO8601().withMessage('Start date must be valid ISO date').toDate(),
    body('endDate').isISO8601().withMessage('End date must be valid ISO date').toDate(),
  ...
  asyncHandler(async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
      ...
      const projectResult = await db.query(
        'SELECT id, name, is_active FROM projects WHERE id = $1',
        [parseInt(String(projectId))] // Ensure integer for project_id
      );
